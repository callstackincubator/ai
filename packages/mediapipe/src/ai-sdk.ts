...
import NativeMediapipeEngine, {
  DownloadProgress,
  type GeneratedMessage,
  type GenerationOptions,
  type Message,
} from './NativeMediaPipe'

export const mediapipe = {
  languageModel: (modelId: string) => {
    return new MediapipeChatLanguageModel(modelId)
  },
}
...
class MediapipeChatLanguageModel implements LanguageModelV2 {
  ...
  public prepare() {
    return NativeMediapipeEngine.prepareModel(this.modelId)
  }

  public async download(progressCallback?: (event: DownloadProgress) => void) {
    const removeListener = NativeMediapipeEngine.onDownloadProgress((event) => {
      progressCallback?.(event)
    })
    await NativeMediapipeEngine.downloadModel(this.modelId)
    removeListener.remove()
  }

  public unload() {
    return NativeMediapipeEngine.unloadModel()
  }

  public remove() {
    return NativeMediapipeEngine.removeModel(this.modelId)
  }
  ...
  async doGenerate(options: LanguageModelV2CallOptions) {
    ...
    const response = await NativeMediapipeEngine.generateText(
      messages,
      generationOptions
    )
  ...
  async doStream(options: LanguageModelV2CallOptions) {
    ...
    const stream = new ReadableStream<LanguageModelV2StreamPart>({
      async start(controller) {
        try {
          const id = (streamId = await NativeMediapipeEngine.streamText(
            messages,
            generationOptions
          ))
          ...
          const updateListener = NativeMediapipeEngine.onChatUpdate((data) => {
            if (data.delta) {
              controller.enqueue({
                type: 'text-delta',
          ...
          const completeListener = NativeMediapipeEngine.onChatComplete(
            (data) => {
              controller.enqueue({
                type: 'text-end',
                id,
      ...
      cancel() {
        cleanup()
        if (streamId) {
          NativeMediapipeEngine.cancelStream(streamId)
        }
      },
    })
